...
console.log('\n📋 GENERATING WITH SEQUENTIAL QUEUE (Fresh refs per prompt)\n');

async function callSeedream(prompt) {
  // Reload refs for each prompt call
  const freshRefs = [
    urlMap['beach_T1_sunset_gaze.jpg'],
    urlMap['beach_T1_sunset_hero.png'],
    urlMap['studio_T1_closeup_smile.png'],
...
(async () => {
  ...
  for (const promptData of OPTIMIZED_PROMPTS) {
    ...
    console.log(`   ⏳ Waiting for API response (fresh references loaded)...`);
    
    try {
      const imageUrl = await callSeedream(promptData.prompt);
      console.log(`   ✅ Generated\n`);
      results.push({ 
        prompt: promptData.letter, 
  ...
  // Save results
  fs.writeFileSync(
    path.join(os.homedir(), 'Downloads/blondeshell_archive 2/outputs/t1_queue_results.json'),
    JSON.stringify(results, null, 2)
  );
  
  console.log('\n✨ Queue complete! All results saved to outputs/t1_queue_results.json\n');
  
  // Print URLs
  results.forEach(r => {
    if (r.url) {
      console.log(`Prompt ${r.prompt}: ${r.title}`);
      console.log(`${r.url}\n`);
    }
  });
})();

...
// Phase 2 helper: get signed URL for a chunk, PUT the bytes, return ETag
async function uploadChunk(uploadId, partNumber, chunkBytes, accessToken, contentType) {
  ...
  if (!urlRes.ok) throw new Error(`Get signed URL failed (part ${partNumber}): ${JSON.stringify(urlBody)}`);

  const signedUrl = urlBody.url ?? urlBody.signedUrl ?? urlBody.signed_url;
  if (!signedUrl) throw new Error(`No signed URL in response: ${JSON.stringify(urlBody)}`);
  ...
  if (!putRes.ok) throw new Error(`Chunk upload failed (part ${partNumber}): ${putRes.status} ${putRes.statusText}`);

  const etag = putRes.headers.get('ETag') || putRes.headers.get('etag');
  if (!etag) throw new Error(`No ETag in PUT response for part ${partNumber}`);
...
export async function uploadMediaBuffer(buffer, filename, mimeType) {
  ...
  for (let i = 0; i < totalChunks; i++) {
    ...
    const etag = await uploadChunk(uploadId, partNumber, chunk, access_token, mimeType);

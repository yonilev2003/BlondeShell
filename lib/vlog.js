...
import { generateTalkingHead } from './hedra.js';
  ...
  const sceneResults = await Promise.all(
    script.scenes.map(async (scene, i) => {
      ...
      if (scene.type === 'talking_head') {
        // 5. Talking head via Hedra
        console.log(`[vlog]   Scene ${i + 1}: Generating talking head...`);
        const talkingHeadUrl = await generateTalkingHead(imageUrl, audioUrl, {
          aspectRatio: '9:16',
          resolution: '1080p',
        });
        const clipPath = join(tmpDir, `scene_${i}.mp4`);
        await downloadToFile(talkingHeadUrl, clipPath);
        return { ...scene, index: i, imageUrl, videoUrl: talkingHeadUrl, clipPath, method: 'hedra' };
      }

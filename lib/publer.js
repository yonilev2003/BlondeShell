...
function buildInstagramPhoto({ caption, mediaId, altText, locationId }) {
  return {
    type: 'photo',
    text: caption,
    media: [{ id: mediaId, type: 'image', ...(altText ? { alt_text: altText } : {}) }],
    ...(locationId ? { details: { location_id: locationId } } : {}),
  };
}

function buildInstagramVideo({ caption, mediaId, postType = 'reel', locationId }) {
  return {
    type: 'video',
    text: caption,
    media: [{ id: mediaId, type: 'video' }],
    details: { type: postType, feed: false, ...(locationId ? { location_id: locationId } : {}) },
  };
}
...
// locationId (optional): Instagram geo-tag. Fetch once via searchInstagramLocation() and reuse.
export async function schedulePost({ accountId, networkKey, caption, scheduledAt, mediaId = null, mediaType = 'photo', postType = null, tiktokTitle = null, locationId = null }) {
  await getWorkspaceId();
  ...
  switch (networkKey) {
    ...
    case 'ig_business':
      networkPayload = mediaType === 'video'
        ? buildInstagramVideo({ caption, mediaId, postType: postType || 'reel', locationId })
        : buildInstagramPhoto({ caption, mediaId, locationId });
      break;
    case 'tiktok':
      networkPayload = mediaType === 'video'
...
// Search Instagram locations via Publer. Returns location object with `id`, or null if not found.
// Cache the id in .env as INSTAGRAM_LA_LOCATION_ID once known.
let _locationCache = new Map();
export async function searchInstagramLocation(query, accountId) {
  if (!query) return null;
  const cacheKey = `${accountId ?? ''}:${query.toLowerCase()}`;
  if (_locationCache.has(cacheKey)) return _locationCache.get(cacheKey);

  await getWorkspaceId();
  const candidates = accountId
    ? [`/accounts/${accountId}/locations?search=${encodeURIComponent(query)}`,
       `/locations?search=${encodeURIComponent(query)}&account_id=${accountId}`]
  ...
  for (const path of candidates) {
    try {
      ...
      const first = list?.[0];
      if (first?.id) {
        const loc = { id: String(first.id), name: first.name ?? query };
        _locationCache.set(cacheKey, loc);
        return loc;
      }
    } catch {
      // try next candidate
  ...
  _locationCache.set(cacheKey, null);
  return null;
}
